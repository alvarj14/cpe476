...
void setVelocity() {
 ...
 /*
  ...
  linear += (distanceToWaypoint*dt);
  proportional = kp*linear;
  
  integral = integral + linear;
  
  derivative = ((linear - old_error)/dt)*kd;
  
  old_error = linear;
  
  double r = proportional + integral + derivative;
  */
 
  // If we are not yet at the waypoint
  while (ros :: ok() && (abs(getDistanceToWaypoint()) > distanceTolerance)) {
      double dist = getDistanceToWaypoint();
      derivative = (dist - old_error)/dt;
      derivative *= kd;
      linear_error += (dist * dt);
      
      double speed = dist + (ki * linear_error)+derivative;
      double ang = atan2(waypointGoal.y - current.y, waypointGoal.x - current.x);
      
      angular_error += (ang * dt);
      
      double angular_speed = kp * ang + ((1 / 100) * angular_error);
      velCommand.linear.x = speed;
      velCommand.angular.z = angular_speed;
	loop_rate.sleep();
          ros :: spinOnce();
  }
}
